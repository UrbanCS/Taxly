...
CREATE TABLE IF NOT EXISTS documents (
  ...
  file_size BIGINT,
  file_type TEXT,
  document_type TEXT, -- 'receipt', 'invoice', 'tax_form', 'bank_statement', etc.
  ...
  extracted_data JSONB, -- Raw OCR/AI extracted data
...
CREATE TABLE IF NOT EXISTS clients (
  ...
  flags TEXT[], -- Array of flags like ['Missing W-2', 'High deductions']
  notes TEXT,
...
CREATE TABLE IF NOT EXISTS tax_calculations (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) NOT NULL,
  name TEXT, -- User-defined name for the calculation
  tax_year INTEGER DEFAULT EXTRACT(YEAR FROM NOW()),
  ...
  calculation_data JSONB, -- Full calculation breakdown
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS email_accounts (
  ...
  sync_folders TEXT[], -- Array of folder names to sync
...
CREATE TABLE IF NOT EXISTS email_documents (
  ...
  document_id UUID REFERENCES documents(id), -- Links to processed document
